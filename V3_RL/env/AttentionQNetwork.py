...
class AttentionQNetwork(nn.Module):
    def __init__(self, stack_input_dim=6, order_input_dim=6, embed_dim=256, n_heads=4, hidden_dim=128):
        super().__init__()
        self.encoder = SelfAttentionEncoder(input_dim=stack_input_dim, embed_dim=embed_dim, n_heads=n_heads)
        self.order_proj = nn.Linear(order_input_dim, embed_dim)
        ...
        # For wait-action score (independent)
        self.wait_mlp = nn.Sequential(
            nn.Linear(order_input_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, 1)
        )
    ...
    def forward(self, order_vec, stack_vecs):
        """
        order_vec: (batch, order_input_dim)
        stack_vecs: (batch, n_stack, stack_input_dim)
        returns:
            action_scores: (batch, n_stack + 1)
        """
        ...
        return action_scores  # (batch, total_actions)
